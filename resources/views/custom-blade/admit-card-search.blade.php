...
<div class="row py-2" id="all-row-py-2">
    ...
    <div class="col-sm-2"><label for="routine">Routine</label><br>
        {{-- <input type="checkbox" value="1" id="routine" checked name="isRoutineSelected" data-style="ios" data-on="Selected" data-off="Not Selected" data-toggle="toggle" data-offstyle="danger" data-onstyle="primary"
        style="padding-top: 9px;"> --}}
        <input type="checkbox" value="1" id="routine" data-on="Select" data-off="Not Select" checked name="isRoutineSelected" data-toggle="toggle" data-onstyle="primary">
    </div>
        ...
        $('#session_id').change(function () {
            ...
            $.get("{{route('student.get-sections')}}",
                ...
                function (data) {
                    ...
                    let secHtml = '<option value="" selected hidden>Select Section</option>';
                    let examHtml = '<option value="" selected hidden>Select Exam</option>';


                    $.each(data.sections, function (i, item) {
                        secHtml += `<option value="${item.id}">${item.class}-${item.shift}-${item.name}</option>`;
                    });
