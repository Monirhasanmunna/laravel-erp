...
<body>
    <div style="width:1100px; margin: auto;padding:0px 10px;margin-bottom:10px;margin-top:10px;height:auto;">
        <table style="width: 100%;">
            ...
            <tr>
                <td colspan="5" style="text-align: center;padding: 5px ;"> <span
                        style="padding: 6px 30px; background-color: white; color: #000;font-size: 14px;font-weight: 600;text-transform: uppercase;border: 1px solid #1e2023;">CLASS ROUTINE</span> </td>
            </tr>
            <tr>
                <td colspan="5" style="text-align: center;padding: 5px ;">
                    Session: {{$data['session']}} , Class: {{$data['class']}} , Category: {{$data['category']}} , Group: {{$data['group']}}
                </td>
            </tr>
        </table>
        ...
        <table style="width: 100%;margin-top:7px;">
            <tr>
                <td>
                    <table style="width: 100%;border: 1px dotted;border-collapse: collapse;text-align:center;padding-right: 3px;">
                        <thead>
                            <tr>
                                <th style="border: 1px dotted #000000;text-transform: uppercase;padding:6px;font-size:15px;">Day</th>
                                @foreach ($data['periods'] as $item)
                                    <th style="border: 1px dotted #000000;text-transform: uppercase;padding:6px;font-size:15px;">{{$item->name}}</th>
                                @endforeach
                            </tr>
                        </thead>
                    ...
                    @forelse ($data['routines'] as $day)
                        <tr>
                            <td align="center" style="border:1px dotted #000000; border-bottom:1px dotted #000000;padding-top:2px;padding-bottom:2px;font-size:15px;">{{$day['day']}}</td>
                            ...
                            @foreach ($day['periods'] as $item)
                                <td align="center" style="border:1px dotted #000000; border-bottom:1px dotted #000000;padding-top:2px;padding-bottom:2px;font-size:15px;">
                                    ...
                                    <p style="font-weight: 400">
                                        {{date("h:i A",strtotime($item['period_time']['start_time']))}} -  {{date("h:i A",strtotime($item['period_time']['end_time']))}}
                                    </p>
                                    @else
