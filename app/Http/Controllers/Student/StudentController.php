...
class StudentController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        $counts          = Student::selectRaw("count(*) as count,session_id, institute_id, class_id, shift_id, section_id")
            ...
            ->with('session', 'ins_class', 'section', 'shift')->orderBy('roll_no', 'ASC')->get();
    ...
    public function getClasses(Request $request)
    {
        ...
        $data['sections']  =  $session->section->map(function ($item) {
                                    return [
                                        'id'    => $item->id,
                                        'name'  => $item->name,
                                        'shift' => $this->getShiftById($item->shift_id),
                                        'class' => $this->getClassById($item->ins_class_id),
                                        'class_id' => $item->ins_class_id
                                    ];
                                });
    ...
    public function getCategoriesGroups(Request $request)
    {
        ...
        $data['categories'] = Category::where('section_id', $request->section_id)->get();
        $data['groups']     = Group::where('section_id', $request->section_id)->get();
        return $data;
    }
    ...
    public function getCatSecGro($class_id, $section_id)
    {
        $data['category'] = Category::where('ins_class_id', $class_id)->where('section_id', $section_id)->get();
        $data['group']    = Group::where('ins_class_id', $class_id)->where('section_id', $section_id)->get();
        return $data;
    }
    ...
    public function getAdmitedStudents(Request $request)
    {
        ...
        $query = Student::with('ins_class', 'section', 'student_user', 'shift', 'group', 'category','testimonial');
    ...
    public function exportPrintStudents(Request $request)
    {
        //return $request->student_ids[0];
        $backendTemplate = $this->backendTemplate['template']['path_name'];
        $students = Student::with('class','session','section','category','group')->whereIn('id', $request->student_ids)->orderBy('roll_no', 'ASC')->get();
        $class = InsClass::find($request->class_id);
        return view($backendTemplate . '.student.student-list-print', compact('students', 'class'));
    }
...
//            ->get();
    ...
    public function getStudentsWithAssignSubjects(Request $request)
    {
        ...
        $query = Student::with('assignSubjects', 'class', 'shift', 'section');
        ...
        $students = $query->get()
            ->map(function ($item) {
                return [
                    ...
                    'otherSubjects'   => $this->getOtherSubjects($item->id),
                ];
            });
    ...
    public function getStudentsWithCommonSubjects(Request $request){
        ...
        $students = $query->get()
            ->map(function ($item) {
                return [
                    ...
                    'section'         => $item->section->name,
                    'roll_no'         => $item->roll_no,
                    'commonSubjects'   => $this->getCommonSubjects($item->id),
                ];
            });
    ...
    public function deleteStudentAssignSubject(Request $request){


        $studentSubjectAssign = StudentSubjectAssign::find($request->id);

        $studentMarksInput =  StudentMarksInputDetail::whereHas('studentMarksInput',function ($q) use($studentSubjectAssign){
                                    $q->where('class_subject_id',$studentSubjectAssign->class_subject_id);
                                })->where('student_id',$request->student_id)->get();

        if($studentMarksInput->count() > 0){
            return response()->json(['message' => "Marks Input Already Assigned"]);
        }

        if ($studentSubjectAssign){
            $studentSubjectAssign->delete();
        }
        return response()->json(['status' => 200]);
    }

    public function getCommonSubjects($studentId){

        $classSubjects = StudentSubjectAssign::with('classSubjects.subject')->where('student_id', $studentId)->get();
        return $classSubjects->chunk(4);
    }
    ...
    public function getOtherSubjects($studentId)
    {
        ...
        $subjectTypeData = [];
        foreach ($subjectTypes as $type) {
            $subjectTypeData[$type->name] = StudentSubjectAssign::with('classSubjects', 'classSubjects.subject')
            ->where('subject_type_id', $type->id)
