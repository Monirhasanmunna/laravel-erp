...
class IdCardController extends Controller
{
    ...
    public function allDownload(Request $request)
    {
        ...
        if(isset($request->check)){
            $student_id = $request->check;
            $students   = Student::whereIn('id', $student_id)->get();
            $ins_idcardTheam = Institution::find(Helper::getInstituteId())->first();

            if($ins_idcardTheam->idcard_theam_id == 1){
               // $pdf = Pdf::loadView($this->backendTemplate['template']['path_name'].'.academic.id-card.download-print-front-1', compact('students'))->setOptions(['defaultFont' => 'sans-serif', 'isHtml5ParserEnabled' => true, 'isRemoteEnabled' => true]);
               return view($this->backendTemplate['template']['path_name'].'.academic.id-card.download-print-front-1', compact('students'));
            }elseif ($ins_idcardTheam->idcard_theam_id == 2) {
                ...
                return view($this->backendTemplate['template']['path_name'].'.academic.id-card.download-print-front-2', compact('students'));
            }elseif ($ins_idcardTheam->idcard_theam_id == 3) {
